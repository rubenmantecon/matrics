			...
			if (res.data.length > 0) {
				...
				for (const item of res.links) {
					...
					if (item.active)
						...
						$("ul.pagination").append(
							`<li class="pageNumber"><a href="${location.pathname
							}?page=${item.url.split("?page=")[1]}">${item.label
							}</a></li>`
						);
				}
			} else {
...
function insertNewRow(...params) {
	...
	if (lastParam == "terms") {
		row += `<td><button id="edit" class="btn save" title="Modificar"><i class="fas fa-pen"></i></button></td>
                <td><a href="/admin/dashboard/${lastParam}/delete/${params[0]
			}" class="btn cancel" title="Elimina"><i class="fas fa-trash"></i></a></td>
                <td><button class="btn save clone" data="${params[params.length - 2]
		...
		row += `<td><a href="/admin/dashboard/students/matriculation${params[params.length - 2]
